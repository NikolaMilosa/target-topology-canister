              ...
              <Row>
                {topologyReport.map((x, k) => (
                  <Col sm={12} md={6} key={k}>
                    <ProductCard params={{ variant: x.violations.length == 0 ? 'success' : 'danger', title: x.limit_name, primaryText: x.limit_value, icon: 'layers', secondaryText: x.violations.length == 0 ? 'Constraint is enforced.' : (
                      <>
